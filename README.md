...
Virtuals → Instant Football → **Play** opens `/virtuals/instant-football`. That list is a **virtual / simulated** fixture board generated by `lib/virtuals/engine.ts`. It is not the live football API and must not be described as real kickoffs.

Tap 1 / X / 2 to add selections to the Instant Football betslip (instant client state). Logged-in users can place a ticket; the selected match then runs through the simulation (Kick Off → chances/shots/goals → Full Time) and settles against the engine score.
