...
## Instant Football (Virtuals)

Virtuals → Instant Football → **Play** opens `/virtuals/instant-football`. That screen is an **interactive demo** recreated with React components to match the Instant Football boards (England, Spain, Germany, Italy, Champions, Euros, Club World Cup). It is not live football, not a screenshot overlay, and not a real-money market.

Tap **1 / X / 2** to highlight an odd and add it to the demo betslip. Enter a stake to see potential return. **Place Bet** writes a demo ticket to this browser only (no wallet debit). The ticket appears under **Open Bets**. Open it and tap **Kick Off** to run a simulated match (goals and scoreline, 1x / 2x, Skip to Result). The ticket then shows **WON** or **LOST** from the simulated score.

These tickets never move real money. Instant V Predict is a separate product and is not used here.
...
- `npm run test` — Vitest (odds, money, phone, football feed mapping, Instant Football virtual engine)
