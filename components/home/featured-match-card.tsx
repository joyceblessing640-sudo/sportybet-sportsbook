...
import { formatKickoff, isGhanaToday, isGhanaTomorrow } from "@/lib/football/time";
...
function kickLabel(match: ClientMatch) {
  ...
  const time = formatKickoff(match.startTime);
  if (isGhanaToday(match.startTime)) return `${time} | Today`;
  if (isGhanaTomorrow(match.startTime)) return `${time} | Tomorrow`;
  return `${time} | ${new Intl.DateTimeFormat("en-GB", { timeZone: "Africa/Accra", day: "2-digit", month: "short" }).format(new Date(match.startTime))}`;
}
    ...
    <article className="w-[92%] min-w-[92%] shrink-0 snap-start bg-white px-3 pb-3 pt-2">
      ...
      <Link href={`/match/${match.id}`} className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
        ...
        <div className="text-center">
          ...
          {market ? <p className="mt-1.5 text-[13px] font-bold text-[#12a150]">1X2</p> : null}
        </div>
        <div className="flex flex-col items-center gap-1.5 text-center">
